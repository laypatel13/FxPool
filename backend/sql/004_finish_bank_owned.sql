-- Remainder of 003 that did not apply: banks + invitation_codes already exist.
-- Run this in the Supabase SQL editor after 003/seed, then retry signup.
...
drop policy if exists "admin_full_banks" on banks;
create policy "admin_full_banks" on banks for all using (
  exists (select 1 from profiles where id = auth.uid() and role = 'admin')
);
drop policy if exists "bank_select_own_bank" on banks;
...
drop policy if exists "exporter_select_own_bank" on banks;
...
drop policy if exists "admin_full_invites" on invitation_codes;
create policy "admin_full_invites" on invitation_codes for all using (
  exists (select 1 from profiles where id = auth.uid() and role = 'admin')
);
drop policy if exists "bank_own_invites" on invitation_codes;
...
drop policy if exists "admin_full_ebr" on exporter_bank_relationships;
create policy "admin_full_ebr" on exporter_bank_relationships for all using (
  exists (select 1 from profiles where id = auth.uid() and role = 'admin')
);
drop policy if exists "bank_own_ebr" on exporter_bank_relationships;
...
drop policy if exists "exporter_own_ebr" on exporter_bank_relationships;
...
drop policy if exists "bank_own_pools" on pools;
...
drop policy if exists "exporter_eligible_pools" on pools;
...
drop policy if exists "bank_own_invoices" on invoices;
