...
def get_overview(user=Depends(require_admin)):
    ...
    active_pools = (
        db.table("pools").select("id", count="exact").in_("status", ["collecting", "suggested"]).execute()
    )
    pending = db.table("pools").select("id", count="exact").eq("status", "suggested").execute()
    executed = db.table("pools").select("id", count="exact").in_("status", ["locked", "settled"]).execute()

    volume_rows = db.table("pools").select("total_amount").in_("status", ["locked", "settled"]).execute()
